...
We assume the data are stored in .pgen/.pvar/.psam format by the [PLINK library](https://www.cog-genomics.org/plink/2.0/formats#pgen). The potential training/validation split can be specified with a separate column in the phenotype file.
...
- `phenotype_file`: the path of the file that contains the phenotype values and can be read as a table.
- `phenotype_names`: names of the phenotype. Must be consistent with the column names in the phenotype file.
- `covariate_names`: a character vector containing the names of the covariates included in the fitting, whose coefficients will not be penalized. The names must exist in the column names of the phenotype file.
...
- `save`: a boolean indicating whether to save intermediate results.
...
There are other parameters that can be used to control the training behavior, such as `weight` to adjust the relative importance of the phenotypes, `p.factor` to control the priority of each variable to be selected by the model.
...
We can also obtain AUC for the specified binary phenotypes.
